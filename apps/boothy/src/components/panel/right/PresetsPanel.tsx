  ...
  const handleApplyPreset = async (preset: Preset) => {
    ...
    try {
      await invoke(Invokes.BoothySetCurrentPreset, {
        ...
        presetAdjustments: preset.adjustments,
      });
    } catch (error) {
      console.error('Failed to set current preset for import:', error);
    }
  };
        ...
        >
          ...
          {!isLoading && presets.length === 0 ? (
            <div className="text-center text-text-secondary py-8 flex flex-col items-center gap-4">
              <p className="max-w-xs">No presets saved yet. Create your own or import from a file.</p>
            </div>
          ) : (
            <>
              <AnimatePresence>
