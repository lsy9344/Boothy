...
- [ ] Task 7: Tests + validation scenarios (AC: 1-9)
  - [x] Manual (MVP gate): verify modal appears within 500ms (manual + T-0); verify each mode exports correct set; verify outputs land in `{session_folder}/Jpg/`; verify end screen appears post-export; verify app remains responsive.
...
- [ ] Regression validation pending (manual export outputs vs pre-change).
