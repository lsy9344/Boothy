...
3. Given free space drops below a **critical threshold**, Boothy blocks booth-critical write actions (capture/transfer ingest/export) in customer mode with a customer-safe message (e.g., “저장 공간이 부족합니다. 직원에게 문의해 주세요.”) and preserves read-only access to already-imported photos.
...
3. **Story 3.3 — Admin Storage Diagnostics + Guided Cleanup**
   - Admin-only panel to view thresholds and current disk stats.
   - Provide recovery actions (open session root; optional: safe deletion of selected old sessions under session root with strict confirmation).
...
- **Primary Risk:** False-positive lockouts (e.g., incorrect drive detection or transient IO errors) blocking booth workflow.
  - **Mitigation:** Determine free space based on the canonicalized session root path; treat IO errors as “unknown” status (warn admin only) rather than hard lockout unless confirmed critical.
...
- UX: customer mode must stay kiosk-safe (“hide not disable”), with minimal and clear messaging; admin mode can show diagnostics.
