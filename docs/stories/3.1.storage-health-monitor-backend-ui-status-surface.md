...
스토리지 모니터가 별도 모듈로 분리되어 있고, 설정 기반 폴링/이벤트 페이로드가 명확합니다. `boothy-storage-health` 이벤트 명명 규칙과 UI 게이팅(고객 모드에서는 warning/critical만 노출)이 스펙과 일치합니다. 스토리 상태는 "Ready for Review"로 표기되어 있어 review 전제("Review")와 형식상 불일치입니다.
...
- Coding Standards: PASS (boothy-* 이벤트, 설정 append-only, async 폴링 적용)
- Project Structure: PASS (docs/architecture/source-tree.md 기준; unified-project-structure.md 없음)
- Testing Strategy: PASS (단위 테스트 추가 + Dev Agent Record 수기 시나리오 검증 기록)
- All ACs Met: PASS (AC1-5 구현 및 수기 검증 기록 기반)

### Requirements Traceability

- AC1: 모니터 루프 + `boothy-storage-health` emit (`storage_health.rs`, `main.rs`) + 수기 검증 기록
- AC2/AC3: 고객 모드 경고 배너 표시/해제 (`App.tsx`)
- AC4: 페이로드 필드(status/free/total/thresholds/sampleAt/diagnostic) 확인 (`storage_health.rs`, `App.tsx`)
- AC5: 샘플 실패 시 unknown + 진단 반환 (`storage_health.rs`) 및 고객 모드 비노출 (`App.tsx`)

### Improvements Checklist

- [x] 스토리지 상태 분류 로직 단위 테스트 확인 (`storage_health.rs`)
- [x] 고객/관리자 모드 UI 게이팅 확인 (`App.tsx`)
- [x] 설정 기본값/역호환 처리 확인 (`file_management.rs`)
- [x] 테스트 실행: `cargo test storage_health` 통과 (경고는 기존 모듈/참조 코드)

### Security Review

새로운 인증/권한 흐름 없음. 진단 문자열은 관리자 모드에서만 표시되며 고객 모드에 기술 정보 노출 없음.

### Performance Considerations

Windows `GetDiskFreeSpaceExW` 호출을 10s 기본 주기로 비동기 루프에서 수행; UI 스레드 블로킹 없음.
...
Gate: PASS - docs/qa/gates/3.1-storage-health-monitor-backend-ui-status-surface.yml
Risk profile: docs/qa/assessments/3.1-risk-20260123.md (not created)
NFR assessment: docs/qa/assessments/3.1-nfr-20260123.md (not created)
