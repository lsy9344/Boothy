...
- `captures[*].renderStatus`:
  ...
  - `previewReady`: published bundle의 `xmpTemplatePath + previewProfile`로 만든 실제 preview file이 존재함. source는 RAW 원본, approved same-capture raster worker output, 또는 Story `1.26` reserve path의 host-owned local native/GPU resident `preset-applied-preview` artifact일 수 있다.
...
- Story `1.26` reserve path에서 host가 same-capture `preset-applied-preview` artifact를 canonical preview path에 닫았다면, 그 시점이 곧 truthful `previewReady` close이며 별도의 per-capture `darktable-cli` preview rerender를 다시 기다리지 않는다.
