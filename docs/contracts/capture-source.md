...
route 결정이 승인되면 그 source가 display-fit preset proxy의 입력이 된다. 그 전에 두 가지가 성립해야 한다.

1. **source가 화면보다 커야 한다.** proxy 렌더는 `--upscale false`로 돈다. source가 작으면 결과도
   작고 display fit gate에서 거부되므로, 판정을 **렌더 전으로** 당긴다.
   렌더 뒤에 버리면 CPU만 태우고 관람 화면은 계속 비어 있다.

   | source | 실측 크기 | 1080p (≈1620×1080) | 4K (≈2880×1920) |
   | --- | --- | --- | --- |
   | CR2 embedded JPEG | 5184×3456 (관측) | 통과 | 통과 |
   | Windows Shell 썸네일 | 요청 1600, **실측은 다를 수 있다** | 조건부 | 미달 |
   | RAW original | 센서 전체 | 통과 | 통과 |

   `IShellItemImageFactory.GetImage`의 `BiggerSizeOk`는 요청보다 큰 캐시본을 허용하므로
   실제 산출 크기가 요청값과 다르다. 표본마다 실측 크기를 기록하게 만든 이유가 이 판정이다.

2. **RAW original은 크기 판정을 미룬다.** 센서 크기를 값싸게 알 수 없으므로 `Unknown`으로
   기록하고 렌더 뒤 gate에 맡긴다. "충분하다"고 가정하지 않는다.

**보정되지 않은 source는 그대로 게시되지 않는다.** `cameraSource`는 진행 피드백일 뿐
qualifying preset-applied frame이 아니다 (FR-010).
...
- [`viewer-display.md`](./viewer-display.md) — Story 7.2·7.4 표시 계약. 이 Story가 깨지 말아야 할 경계
- [`render-worker.md`](./render-worker.md) — Story 7.4 display-fit proxy 렌더 규칙
