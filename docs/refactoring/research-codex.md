...
작성일: 2026-03-07
최근 동기화: 2026-03-08 PRD v0.3 camera connection status sync
작성자: Codex  
상태: Superseded supporting research for archived redesign rationale
문서 상태: 이 문서는 이전 redesign work order 내용을 흡수한 연구 메모이며, 현재는 supporting research로 보존한다.
문서 우선순위: 현재 구현 기준 문서는 `_bmad-output/planning-artifacts/architecture.md`와 `_bmad-output/planning-artifacts/research/technical-boothy-wrapping-efficiency-research-2026-03-08.md`다. `docs/refactoring/research-codex.md`는 source-level rationale과 extraction 근거를 보존하는 보조 문서로만 취급한다.
...
이 문서는 `docs/refactoring/` 아래 리팩터링 설계 연구를 하나로 합친 결과물이다.

- 목적은 Boothy의 greenfield rebuild 방향을 한 문서에서 결정 가능하게 만드는 것이다.
- 범위는 문서 재설계와 아키텍처 판단까지이며, 제품 코드 구현은 포함하지 않는다.
- 현재는 최종 아키텍처와 기술 리서치가 별도 planning artifact로 고정되었으므로, 이 문서는 reference-level 근거와 초기 판단 맥락을 보존하는 용도로만 유지한다.
...
2026-03-08 PRD v0.3에서는 카메라 연결상태를 고객 화면과 운영자 화면 모두에 명시적으로 반영했다.

- 고객 화면은 내부 진단값 대신 `연결 확인 중 / 촬영 가능 / 전화 필요`에 해당하는 고객 친화 문구를 보여줘야 한다.
- 운영자 화면은 고객에게 현재 어떤 연결 안내가 보이는지와 운영자 판단용 연결상태를 함께 대조할 수 있어야 한다.
- 따라서 이 문서의 Host/UI 가치 판단과 boundary rationale도 "카메라 연결상태를 양쪽 화면에 어떻게 다르게 번역할 것인가"를 포함하도록 읽어야 한다.
...
- 고객은 시작 준비 구간과 예외 구간에서 카메라 연결상태를 기술 용어 없이 이해해야 한다.
- 고객은 프리셋 선택, 최신 사진 확인, 진행 중 결과 누적, 종료 후 인계 흐름을 자연스럽게 경험해야 한다.
- 고객은 `내보내기 중`, `전화 필요`, `세션명 인계` 같은 결과 중심 상태만 이해하면 된다.
- 운영자는 복잡한 SDK 상태 전체가 아니라 `촬영 가능 / 준비 중 / 전화 필요` 같은 압축된 운영 신호와 최근 원인, 그리고 고객에게 현재 노출 중인 카메라 연결 안내를 빠르게 파악하면 된다.

즉, 지금 제품에서 가장 값비싼 자산은 "카메라를 어떻게 직접 만지느냐"보다 "고객 여정과 운영 여정을 어떻게 안정적으로 보여주느냐"다.
카메라 연결상태를 고객용 문구와 운영자용 진단 신호로 각각 번역하는 책임도 이 Host/UI 가치에 포함된다.
...
- **operator-visible diagnostics**: customer-facing copy와 operator-only 상태를 함께 보여주는 translation layer

이 원칙은 다시 깨면 안 된다.
Host와 UI는 camera internals를 재판정하는 계층이 아니라, 이미 정규화된 신호를 번역하는 계층이다.
특히 카메라 연결상태는 고객용 문구와 운영자용 판단 상태가 같은 engine truth에서 파생되어야 하며, 어느 화면도 자체 추론으로 연결 성공 여부를 다시 결정하면 안 된다.
...
- 고객 상태는 PRD 기준의 작은 상태 집합으로 유지한다: `준비 중`, `촬영 가능`, `내보내기 중`, `완료`, `전화 필요`
- 고객 화면의 카메라 연결상태는 PRD 기준으로 `카메라를 연결하고 있습니다. 잠시만 기다려 주세요.`, `카메라가 연결되어 촬영을 시작할 수 있습니다.`, `카메라 연결이 확인되지 않습니다. 전화해 주세요.`처럼 다음 행동이 분명한 문구로만 번역한다.
- 운영자 표시는 PRD에 맞춰 `촬영 가능 / 준비 중 / 전화 필요` 같은 압축 신호로 번역하되, 고객에게 현재 어떤 연결 안내가 노출되는지와 운영자 판단용 `카메라 연결 상태`를 함께 보여줄 수 있어야 한다.
- 그러나 그 뒤에는 두 화면이 공유하는 더 풍부한 diagnostics 모델이 별도로 존재해야 한다.
...
2. `_bmad-output/planning-artifacts/research/technical-boothy-wrapping-efficiency-research-2026-03-08.md`
3. `_bmad-output/planning-artifacts/prd.md`
4. `docs/refactoring/research-codex.md`
...
- 현재 기준 문서는 `architecture.md`와 technical research artifact다.
- 이 문서는 삭제 대상이 아니라 supporting research archive로 유지한다.
- 과거 `refactoring/research.md` 계열 문서가 복원되더라도 historical alternatives로만 읽는다.
- 현재 체크아웃에는 예전 문서가 언급하던 `apps/boothy/*`, `apps/camera-sidecar/*`, `work/*` 경로가 없다.
- 따라서 future LLM은 planning artifacts를 먼저 기준으로 잡고, 이 문서는 Canon extraction seam이나 초기 boundary rationale이 필요할 때만 보조 근거로 읽는다.

## 8. Acceptance Checks

이 문서는 supporting rationale 문서로서 아래 질문에 답할 수 있어야 archive 가치가 있다.

1. 현재 기본 권고 아키텍처 경로는 무엇인가?
2. 왜 이 경로가 현재 sidecar 안정화보다 더 효율적인가?
3. 왜 Photino-first 또는 전체 native rewrite보다 더 효율적인가?
4. Canon-focused extraction 근거가 되는 정확한 reference 파일은 무엇인가?
5. 왜 `CameraDeviceManager` 전체는 제품 베이스로 보기에는 너무 넓은가?
6. RapidRAW에서 재사용할 수 있는 것과 그대로 가져오면 안 되는 것은 각각 무엇인가?
7. 실제 구현 전에 먼저 검증해야 할 첫 spike path는 무엇인가?
8. 임시 학습 scaffold와 최종 product architecture를 어떻게 구분해야 하는가?
9. 고객 화면과 운영자 화면은 같은 카메라 연결 truth를 어떻게 다르게 번역해야 하는가?
