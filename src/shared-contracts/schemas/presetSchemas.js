import { z } from 'zod';
import { presetCatalog as approvedPresetCatalog, presetIdSchema, } from '../presets/presetCatalog.js';
export const presetCatalogGroupSchema = z.enum(['tone', 'background']);
const approvedPresetNameById = new Map(approvedPresetCatalog.map((preset) => [preset.id, preset.name]));
const approvedPresetOrderById = new Map(approvedPresetCatalog.map((preset, index) => [preset.id, index]));
