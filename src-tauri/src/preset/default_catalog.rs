...
const DEFAULT_PRESET_SEEDS: [DefaultPresetSeed; 3] = [
    (
        ...
        &["temperature", "exposure", "sigmoid", "bloom", "sharpen"],
    ),
    (
        "preset_mono-pop",
        "2026.03.27",
...
pub fn ensure_default_preset_catalog_in_dir(base_dir: &Path) -> Result<(), HostErrorEnvelope> {
    ...
    let has_existing_default_seed =
        DEFAULT_PRESET_SEEDS
            .iter()
            .any(|(preset_id, published_version, _, _, _, _)| {
                catalog_root
                    .join(preset_id)
    ...
    ) in DEFAULT_PRESET_SEEDS
    {
        ...
        let bundle = json!({
            ...
            "proxyPublication": {
                ...
                "proxyRecipeVersion": published_version,
                "proxyRecipePath": "xmp/template.xmp",
                ...
                "visualApproval": {
                    "approvedAt": "2026-08-14T00:00:00+09:00",
                    "approvedBy": "Noah Lee (manual product approval)",
                },
            },
            "preview": {
                "kind": "preview-tile",
...
fn bundle_requires_runtime_backfill(bundle_dir: &Path) -> bool {
    ...
    load_published_preset_summary(bundle_dir).is_none()
        || load_published_preset_runtime_bundle(bundle_dir)
            .map(|bundle| !bundle.has_approved_visual_parity())
            .unwrap_or(true)
}
