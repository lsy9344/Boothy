...
pub struct PresentSampleRecord {
    ...
    /// Story 7.4. 이 표본이 어느 tier의 표시였는가 (`sample` / `displayFitPresetProxy`).
    /// **fixture 표본과 실제 preset 표본을 한 파일 안에서 구분하는 유일한 근거다.**
    #[serde(default)]
    pub tier: Option<String>,
    /// Story 7.4. proxy 표본의 출처 요약. sample 표본에서는 전부 `None`이다.
    #[serde(default)]
    ...
    /// Story 7.4의 proxy 구간 진단 span. **어느 것도 KPI 종료점이 아니다.**
    #[serde(default)]
    pub proxy_source_ready_at_micros: Option<u64>,
    /// 렌더 큐 슬롯을 기다린 시간. preview/final과 슬롯 2개를 공유해서 생긴다.
    /// 이 값이 크면 Story 7.6의 우선순위 스케줄러가 필요하다는 증거다.
    #[serde(default)]
    pub proxy_queue_wait_micros: Option<u64>,
    #[serde(default)]
    pub proxy_render_start_at_micros: Option<u64>,
    /// darktable 프로세스가 **종료한** 시각. AC 3의 첫 관문이다.
    #[serde(default)]
    pub proxy_process_exited_at_micros: Option<u64>,

    /// Story 7.6. 이 프레임의 렌더 품질 (`fast` / `high`).
    ///
    /// `tier`와 함께 실어 **잘못 표기된 행을 evidence에서 잡아낼 수 있게** 한다.
    /// 두 tier는 다른 어떤 필드로도 구분되지 않는다.
    #[serde(default)]
    pub render_quality: Option<String>,

    /// Story 7.6의 RAW 정밀본 구간 진단 span. **어느 것도 KPI 종료점이 아니다.**
    ///
    /// 공식 KPI는 그대로 `trusted input → 첫 자격 frame actual present`다.
    /// 정밀본 present는 **두 번째 terminal 행**이지 KPI 종료점이 아니며,
    /// 5초를 넘어도 그대로 기록한다.
    #[serde(default)]
    pub raw_refined_enqueued_at_micros: Option<u64>,
    #[serde(default)]
    pub raw_refined_queue_wait_micros: Option<u64>,
    #[serde(default)]
    pub raw_refined_render_start_at_micros: Option<u64>,
    #[serde(default)]
    pub raw_refined_process_exited_at_micros: Option<u64>,
    #[serde(default)]
    pub raw_refined_committed_at_micros: Option<u64>,
    /// 정밀본이 실제로 화면에 올라간 시각. 이 행이 정밀본 generation일 때만 값이 있다.
    #[serde(default)]
    pub raw_refined_presented_at_micros: Option<u64>,

    /// Story 7.6의 스케줄러 구간. **큐 대기가 전체 지연에서 차지하는 비율**을
    /// evidence에서 계산할 수 있게 하는 값들이다 (HV-17A).
    #[serde(default)]
    pub scheduler_priority: Option<String>,
    #[serde(default)]
    pub scheduler_deadline_micros: Option<u64>,
    /// deadline을 넘겨서 시작했는가. **관측일 뿐 중단 사유가 아니다.**
    #[serde(default)]
    pub scheduler_deadline_missed: Option<bool>,
    #[serde(default)]
    pub scheduler_coalesced_count: Option<u32>,
    #[serde(default)]
    pub scheduler_preempted_by: Option<String>,
