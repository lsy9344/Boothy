...
| 7.6 | Pass | Real EOS 700D partial run; both sub-gates incomplete | No-Go / Blocked | RAW-only recovery produced 5/5 CR2 captures and 5/5 proxy presents with zero P0 dropouts, but no refined generation, no four cancellation rounds, no slanted-edge measurements, and no observer trial. Rerun prerequisites are recorded; the lane stays off. | Codex (operator: Noah Lee) | `tests/hardware/raw-refined/run-20260817-003242-hv17/` |
...
- evidence package path: `tests/hardware/raw-refined/run-20260817-003242-hv17/` (real EOS 700D run; preflight recovery in `run-20260816-235448-hv17/`)
- Go / No-Go result: `HV-17A No-Go`; `HV-17B No-Go`; product disposition `Blocked / rerun required`
- release blocker: `RAW-only recovery produced 5/5 real CR2 captures, 5/5 proxy commits/presents, zero P0 dropouts, and 9/22/22 μs queue-wait p50/p95/max. HV-17A still lacks the four mandatory cancellation/process-tree rounds. HV-17B produced no refined generation because the tier remains NotMeasured, and the captured scene was too dark and lacked a slanted edge; the 9-pair measurements, transition recording, 3-observer detection trial, and Noah Lee sign-off therefore do not exist. The refined lane remains disabled by default.`
- follow-up owner: `Noah Lee`
- rerun prerequisite: `Keep the EOS 700D in verified RAW-only mode; provide normal lighting and a clear slanted-edge/ratio chart; generate at least 3 real captures × all 3 approved presets as proxy/refined pairs; execute the four cancellation rounds with raw taskkill/orphan evidence; provide a real swap recording and 3 named observers for the detection test.`
- target rerun date: `After lighting/target and three observers are available`
