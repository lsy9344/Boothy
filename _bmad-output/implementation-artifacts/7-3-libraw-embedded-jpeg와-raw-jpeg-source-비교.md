...
- [ ] **품질 corpus**: 대표 촬영의 route별 산출물을 나란히 비교한 이미지 세트. 노출/색/선명도/artifact를 사람이 승인한다.
- [ ] **AB/BA 집계**: route별 p50/p95/max, 성공률, source 도착 순서 분포, 실패 모드. **실패와 timeout을 제외하지 않는다.**
- [ ] **route 결정**: primary / fallback / No-Go와 근거. No-Go여도 조사 목적은 닫을 수 있다.
...
- [ ] `_bmad-output/implementation-artifacts/hardware-validation-ledger.md`의 Story 7.3 행을 갱신한다.
- [ ] HV-14가 route 결정을 기록한 뒤에만 status를 `done`으로 전환한다. **자동 테스트 통과만으로 `done` 처리하지 않는다.**
