...
- same-capture first-visible image가 준비되면 최신 사진 레일에 먼저 노출할 수 있지만, preset-applied truthful close가 준비될 때까지 상태 자체는 `Preview Waiting`으로 유지합니다.
- first-visible source가 무엇이든 고객에게는 "먼저 같은 컷이 보이고, 나중에 선택한 룩의 truthful 결과로 안정화된다"는 경험이 유지되어야 합니다.
...
- **인터랙션:** 가능하면 방금 찍은 same-capture 확인 이미지를 먼저 보여주고, 내부 source가 무엇이든 고객에게는 같은 컷이 먼저 보인다고 이해되게 유지합니다. 이후 booth-safe preset-applied truthful preview가 준비되면 같은 자리에서 자연스럽게 교체합니다.

#### ### 프리뷰 대기 패널 (Preview Waiting Panel)
- **목적:** 촬영 저장 사실과 프리뷰 준비 상태를 분리해 고객 신뢰 유지.
- **구조:** 저장 완료 배지 + 대기 메시지 + 선택적 보조 문구 + 현재 가능한 다음 행동.
- **인터랙션:** first-visible source가 무엇이든 상태 copy는 `Preview Waiting`을 유지하고, truthful preset-applied preview가 준비되면 자연스럽게 최신 사진 레일 또는 리뷰 상태로 이어집니다.
