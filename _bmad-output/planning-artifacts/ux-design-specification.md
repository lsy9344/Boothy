...
### Project Vision
Boothy는 복잡한 편집 과정 없이, 전문가가 설계한 프리셋을 통해 누구나 고품질의 사진을 빠르고 즐겁게 촬영할 수 있는 윈도우 기반 포토 부스 솔루션입니다. 사용자의 시간을 존중하며, 명확한 가이드와 감각적인 디자인(Brutal Core)으로 신뢰감 있는 촬영 경험을 제공합니다.

### Target Users
- **현장 고객:** 빠른 촬영 시작과 확실한 결과물을 원하는 일반인.
- **프리셋 제작자:** RapidRAW 엔진을 사용하여 부스 전용 프리셋을 설계하는 전문가.
- **시스템 운영자:** 장비 상태를 모니터링하고 필요 시 복구 작업을 수행하는 관리자.

### Key Design Challenges
- **시간 관리의 명확성:** 쿠폰 연동 시간과 종료 예고 사운드 알림을 통한 사용자 안내.
- **선택의 단순화:** 상세 편집 대신 프리셋 카탈로그 중심의 직관적 UX.
- **신뢰성 있는 피드백:** 촬영 즉시 결과물을 확인하고 세션 내에서 안전하게 관리하는 흐름.
...
### 구속되는 UX 요구사항
- 고객 경험은 booth-first, preset-driven 흐름을 유지해야 하며 고객에게 직접 편집 워크스페이스나 내부 저작 제어를 노출하지 않습니다.
...
- 고객용 문구는 plain-language와 낮은 문구 밀도 원칙을 지키고 기술 진단어 또는 내부 저작 용어를 포함하지 않습니다.
- 부스 화면은 고대비, 멀티모달 알림, 터치 친화적 조작을 유지해야 합니다. 넉넉한 터치 영역은 필수이며 `80px`은 본 문서의 설계 기준선입니다.

### 설계 가이드 / 지향점
- 촬영 전 프리셋 효과를 라이브로 보여주는 방식은 권장 표현 중 하나이지만, 현재 PRD나 아키텍처가 강제하는 릴리스 게이트는 아닙니다.
- 매우 빠른 프리셋 전환, 프리뷰와 최종 결과의 높은 유사성, exact progress tracker 표현, 축하 애니메이션, 특정 컴포넌트 배치는 지향점이지 계약 요구사항은 아닙니다.
- 구현상 트레이드오프가 필요하면 booth-first, preset-driven, Brutal Core 방향과 고객 편집 비노출 원칙을 우선 유지합니다.

## 2. Core User Experience

### 2.1 Defining Experience
Boothy의 정의적 경험은 **'프리셋 우선 촬영(Filter-First Capture)'**입니다. 사용자가 촬영 전에 "완성된 룩을 골랐다"는 확신을 얻고 자신감 있게 셔터를 누르는 순간이 Boothy의 핵심 가치입니다. 이 확신은 대표 프리뷰 타일, 활성 프리셋 표시, 최신 사진 피드백, 또는 구현 가능한 범위의 라이브 미리보기로 형성될 수 있으며, 복잡한 후보정 대신 '완성된 룩'을 선택하는 즐거움에 집중합니다.

### 2.2 User Mental Model
사용자는 Boothy를 '나를 가장 예쁘게 보여주는 스마트한 거울'로 인식합니다. 복잡한 후보정 단계가 나중에 있을 것이라 기대하기보다, 지금 선택한 프리셋의 성격과 최신 사진 피드백이 이후 결과물에 일관되게 반영될 것이라는 정신 모델을 가집니다. 이 문서는 픽셀 단위의 100% 사전 일치를 계약으로 요구하지는 않지만, 사용자가 속았다고 느끼지 않을 정도의 룩 일관성은 유지해야 한다고 봅니다.

### 2.3 Success Criteria
**구속되는 UX 요구사항**
- **프리셋 중심 흐름:** 사용자는 승인된 프리셋을 고르고 직접 편집 없이 촬영을 이어갈 수 있어야 합니다.
...
1. **진입:** 세션 시작 직후 직관적인 프리셋 선택 화면 노출. 대형 카드 형태로 "룩을 골라보세요"라고 제안합니다.
2. **상호작용:** 프리셋 카드 터치 시 선택된 룩이 즉시 인지되어야 합니다. 이는 활성 상태 강조, 대표 프리뷰 타일, 또는 구현 가능한 범위의 라이브 미리보기로 표현할 수 있으며, 사용자는 가장 마음에 드는 순간 촬영 버튼을 누릅니다.
3. **피드백:** 촬영 즉시 프리셋이 적용된 결과물이 화면 하단 썸네일 레일에 나타나며 "잘 찍혔어요!"라는 확신을 줍니다.
4. **마무리:** 쿠폰 연동 시간 종료 5분 전 사운드 알림으로 안내하며, 종료 시 데이터를 안전하게 저장하고 세션을 마무리합니다.
...
- **배제:** 전문가용 편집 기능을 과감히 제거하고, 사용자의 창의적 선택을 '프리셋 카탈로그' 내로 한정하여 속도감 있는 경험을 만듭니다.
...
- **Touch Targets:** 서서 조작하는 환경을 고려해 넉넉한 터치 영역이 필요합니다. `80x80px`은 기본 설계 기준선이며, 실제 구현은 승인된 하드웨어 검증 결과에 따라 소폭 조정할 수 있습니다.
...
사용자가 부스에 들어와 세션명을 입력하고 프리셋을 선택하여 촬영 준비를 마치는 과정입니다.

```mermaid
flowchart TD
    A[입구 화면: START 터치] --> B[세션 이름 입력]
    B -->|입력 완료| C[프리셋 카탈로그 노출]
    C --> D{프리셋 선택}
    D -->|선택 시| E[활성 프리셋 상태 확인]
    E --> F[준비 완료 확인]
    F --> G[촬영 화면으로 이동]
```

### 촬영 루프 및 실시간 확인 (Capture Loop)
선택한 프리셋으로 사진을 촬영하고 즉시 결과물을 확인하며 만족감을 얻는 핵심 반복 과정입니다.
...
flowchart TD
    ...
    B --> C[카운트다운 및 촬영]
    C --> D[프리셋 적용 이미지 생성]
    D --> E[하단 레일에 최신 사진 업데이트]
    E --> F{계속 촬영?}
    F -->|YES| B
    F -->|프리셋 변경| G[프리셋 카탈로그 호출]
    G --> B
    F -->|사진 삭제| H[현재 세션 사진 삭제]
    H --> F
```
...
- **상시 상태 공유:** 현재 세션명과 타이밍 정보, 촬영 관련 핵심 상태는 화면 안에서 쉽게 인지 가능해야 합니다. 이를 상단 배지, 헤더, 고정 영역 등 어떤 패턴으로 표현할지는 구현에서 조정할 수 있습니다.
...
- **터치 우선 설계:** 서서 조작하는 환경에 맞춰 모든 인터랙션 요소는 넉넉한 터치 영역을 유지해야 하며, `80px`은 기본 기준선으로 취급합니다.

### Implementation Roadmap
- **Phase 1:** 촬영 루프 필수 요소 (카메라 패널, 셔터, 프리셋 카드)
- **Phase 2:** 세션 관리 요소 (시간 배지, 입력 필드, 삭제 모달)
- **Phase 3:** 경험 완성 요소 (사진 레일, 축하 애니메이션)
...
### Form Patterns
세션명 입력 등 핵심 입력 폼은 부스 환경에서 오조작을 줄일 만큼 충분한 터치 영역을 확보해야 합니다. `80px`은 권장 기준선입니다.
- **Validation:** 입력 즉시 유효성을 검사하여 하단에 명확한 텍스트로 가이드합니다.

### Navigation Patterns
- **Session Header:** 세션명과 시간 정보(조정된 종료 시각 또는 이에 준하는 명확한 타이밍 정보)는 화면 안에서 쉽게 인지 가능해야 합니다. 고정 위치와 시각 표현은 구현에서 조정 가능합니다.
...
- **Mobile:** 767px 이하 (제한된 상태 공유 전용)

### Accessibility Strategy
WCAG 2.2 AA 수준을 기본 접근성 목표로 하여 누구나 소외 없는 촬영 경험을 제공합니다.
- **High Contrast:** 따뜻한 베이지와 블랙의 고대비 조합 유지.
- **Giant Touch Targets:** 모든 핵심 버튼은 부스 환경에 맞는 넉넉한 터치 영역을 유지해야 합니다. `80px`은 기본 기준선입니다.
