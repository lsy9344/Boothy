...
FR4: 고객은 현재 세션에 사진을 촬영해 안전하게 저장할 수 있어야 하며, latest large preview replacement가 아직 preset-applied truthful close에 도달하지 않았더라도 저장 성공, first-visible reassurance, `Preview Waiting` truth를 구분해서 안내받아야 한다.
FR5: 고객은 현재 세션의 사진만 검토할 수 있어야 하고, `Current-Session Deletion Policy`가 허용하는 범위에서만 삭제할 수 있어야 하며, 프리셋은 세션 중 언제든 변경할 수 있어야 한다. 변경은 이후 촬영부터만 반영되고 이미 저장된 촬영본은 capture 시점의 preset-version binding을 유지해야 한다.
...
NFR3: 주요 고객 액션은 1초 이내에 응답이 인지되어야 하며, 성공적으로 저장된 촬영의 same-capture `first-visible`과 `latest preset-applied large preview replacement`는 분리된 지표로 추적되어야 한다. booth surface는 truthful close가 끝날 때까지 host-owned `Preview Waiting`을 유지해야 한다.
...
- Rust 호스트는 카메라 상태, 타이밍 상태, latest large preview replacement, 사후 완료 상태를 정규화하는 단일 진실 계층이어야 하며, React는 정규화된 상태만 소비해야 한다.
...
- 프리셋은 불변 게시 번들로 저장되어야 하고, 활성 세션과 각 capture record는 정확한 프리셋 버전을 참조해야 하며, 게시/롤백은 미래 세션에만 영향을 줘야 한다.
- `Preview Waiting`, `previewReady`, `readyAtMs`, latest large preview promotion은 모두 호스트가 소유하는 진실이어야 하며, UI는 이를 추정하거나 독자적으로 승격하면 안 된다.
- first-visible artifact와 truthful close artifact는 같은 capture, 같은 canonical latest-preview slot/path에서 연결되어 later same-slot replacement로 닫혀야 한다.
- latest large preview는 booth preview의 주 artifact여야 하며, current-session rail은 별도 진실 경로를 만들지 않고 같은 close owner를 공유하거나 그 산출물에서 파생돼야 한다.
...
FR4: Epic 1 - 현재 세션 저장 성공, host-owned `Preview Waiting`, latest large preview replacement truth
FR5: Epic 2 - 현재 세션 검토, 삭제, 세션 중 프리셋 변경과 forward-only preset-version binding
...
### Epic 1: 빠른 세션 시작과 latest preview replacement 신뢰
고객이 이름+휴대전화 뒤4자리로 빠르게 세션을 시작하고, 승인된 프리셋을 고른 뒤, 준비 상태를 이해하며 첫 촬영을 성공적으로 저장하고, host-owned `Preview Waiting` 안에서 latest preset-applied large preview replacement를 신뢰할 수 있게 한다.
**FRs covered:** FR1, FR2, FR3, FR4

### Epic 2: 현재 세션 review truth와 시간 인지
고객이 현재 세션 사진만 검토하고 정책 범위 내에서 삭제하며, latest preview와 rail이 같은 host-owned truth를 공유한 채 세션 중 프리셋을 바꿔도 이미 저장된 촬영은 capture-bound preset version을 유지하면서 촬영을 이어갈 수 있게 한다.
**FRs covered:** FR5, FR6
...
### Story 1.5: 현재 세션 촬영 저장과 host-owned `Preview Waiting`

As a booth customer,
I want capture success, first-visible reassurance, and preset-applied preview readiness to stay clearly separated,
So that I know my photo is saved without mistaking an early image for the final truthful booth preview.
...
**And** the capture record stores the capture-bound preset identity and published version for later preview replacement and final render truth
...
**And** the next message explains that the latest large preview is still being prepared and what the customer can do next
**And** host-owned `Preview Waiting` remains the only preview truth until the preset-applied artifact for that same capture and preset version is validated

**Given** a successful capture is acknowledged
**When** the booth reports first-visible progress and later truthful close on approved hardware
**Then** the primary customer action is acknowledged within 1 second
**And** telemetry distinguishes `fastPreviewVisibleAtMs` from `previewVisibleAtMs` for that same capture
**And** the booth remains in truthful `Preview Waiting` until the latest preset-applied large preview replacement actually closes

**Given** the booth is in `Preview Waiting`
**When** a same-capture first-visible image arrives before truthful close
**Then** the booth may surface that pending image on the canonical latest-preview surface and any derived rail view
**And** it does not raise `previewReady`, `readyAtMs`, or imply that preset-applied preview truth has already closed

**Given** the booth is in `Preview Waiting`
**When** the latest-preview slot or current-session rail is still empty
...
### Story 1.8: 게시된 프리셋 버전 적용과 host-owned preview/final truth

As a booth customer,
I want the preset I selected to be actually applied to the latest large booth preview and final output,
So that the look I chose matches both the truthful booth close and what is later delivered.
...
**And** `previewReady` is recorded only after a real preset-applied large preview file exists at the canonical latest-preview slot under `renders/previews/`

**Given** preview render succeeds for a booth capture
**When** the main latest-preview surface and any derived rail update
**Then** they use the preset-applied capture preview as the close owner rather than a raw copy, placeholder fallback, or bundle representative tile
...
- 이 스토리는 preset-applied `previewReady` / `finalReady` truth와 latest large preview close의 소유자다.
...
As a booth customer,
I want my just-captured photo to appear through the canonical latest-preview slot as quickly as safely possible even before the preset-applied preview is fully ready,
...
**And** any rail surface that uses that image does so as a shared or derived artifact rather than an independent preview truth path
...
**Then** it replaces the pending image at the same canonical preview path for the same capture and preset version
...
**Then** telemetry separates `fastPreviewVisibleAtMs` from `previewVisibleAtMs`
**And** hardware evidence confirms same-slot replacement correctness, preset-version binding, close-owner continuity, cross-session isolation, and safe behavior during burst capture queue delay
...
As a booth customer,
I want my same-capture first-visible image to reach the canonical latest-preview slot at product-acceptable speed without destabilizing later truthful replacement,
...
**When** the runtime prepares first-visible work for the canonical latest-preview slot on approved booth hardware
...
**Then** the session-level seam log contains `request-capture`, `file-arrived`, `fast-preview-visible` or equivalent first-visible event, route-selection evidence, truthful-close-owner evidence, `capture_preview_ready`, and the later canonical latest-preview replacement event
...
**Then** the canonical latest-preview slot and same-slot replacement rule remain unchanged
...
Story 1.10 scope note:
- 이 스토리는 `first-visible lane` 안정화, per-session seam completeness, truthful fallback baseline을 마무리하는 precondition corrective track이다.
- `preset-applied truthful close`의 새 renderer route 실험과 canary routing은 후속 Story 1.11이 맡는다.

### Story 1.11: local dedicated truthful close route와 canary routing

As a booth customer,
I want the booth to close my preset-applied latest preview through the fastest approved route without weakening truth,
...
**Given** the host keeps capture-bound preset identity, session-scoped paths, and same-slot replacement rules
**When** preview-close execution is routed through an approved local dedicated renderer sidecar
**Then** that sidecar acts only as a candidate-result producer behind the host
**And** render truth remains host-owned rather than sidecar-owned

**Given** the sidecar returns a candidate canonical preview output for the current capture
**When** the host validates same-session, same-capture, allowed-path, and preset-version correctness
**Then** the host may accept that output as the truthful preset-applied latest-preview close
**And** `previewReady` is never raised before host validation succeeds
**And** the large preview surface and any rail derivatives share that accepted close owner rather than inventing a second truth path

**Given** the booth runtime supports more than one approved preview-close route
**When** the host selects a route for a booth, session, or preset
**Then** routing is controlled through explicit feature-gated policy
**And** unhealthy sidecar execution can fall back immediately to the approved darktable path without breaking customer-safe waiting behavior

**Given** the customer has already seen a same-capture first-visible image in `Preview Waiting`
**When** the sidecar route or fallback route completes the truthful preset-applied preview
**Then** the same-slot replacement rule remains unchanged
**And** the customer still sees no new state name beyond truthful `Preview Waiting` and later ready states

**Given** hardware canary validation is run on approved booth hardware
**When** the team compares local dedicated renderer routing with darktable fallback
**Then** diagnostics show selected route, fallback reason, close-owner result, elapsed timing, and fidelity evidence inside one session package
**And** approval requires meaningful truthful preview close improvement with `false-ready` and cross-session leakage both remaining at zero

Story 1.11 scope note:
- 이 스토리는 `full platform pivot`이 아니라 `host truth 유지 + routeable local renderer topology` 실험을 소유한다.
- darktable fallback, canary rollout, 즉시 rollback은 이 스토리의 필수 범위다.
...
**And** the rail does not become an independent preview truth path apart from the canonical latest-preview slot for that capture
...
**And** any surfaced latest image shares or derives from the same host-owned close owner used for the latest booth preview
...
**And** any capture already saved or already inside `Preview Waiting` keeps its original capture-bound preset-version binding through later preview and final close
...
**And** later same-slot replacement for older captures continues to close against the preset version that was bound when each capture was taken
