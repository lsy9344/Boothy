...
FR5: 고객은 현재 세션의 사진만 검토할 수 있어야 하고, `Current-Session Deletion Policy`가 허용하는 범위에서만 삭제할 수 있어야 하며, 프리셋 변경은 이후 촬영분에만 적용되어야 한다.
...
FR5: Epic 2 - 현재 세션 검토, 삭제, 이후 촬영용 프리셋 변경
...
고객이 현재 세션 사진만 검토하고 정책 범위 내에서 삭제하며, 이후 촬영용 프리셋을 바꾸고, 조정된 종료 시각과 경고 알림을 이해하면서 촬영을 이어갈 수 있게 한다.
**FRs covered:** FR5, FR6
...
**Given** an active session with no capture yet
**When** the customer reaches preset selection
...
**When** the customer selects one preset
**Then** that preset becomes the active preset for the session
**And** the active preset identity and published version are stored for later captures
...
### Story 2.3: 이후 촬영용 활성 프리셋 변경

As a booth customer,
I want to change the active preset during my session,
So that future photos can use a different approved look without changing past captures.
